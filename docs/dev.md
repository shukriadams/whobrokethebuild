...
Post processors are amongst the last plugins run on a build event. They can be used to determine causes of build failures, using log extractions generated by parsers. Typically, several post processors are run on a build, and the first one to "declare" a conclusive result wins and gets to flag the build. Post processors should therebefore be run in order of decreasing reliability and specificity.