        ...
        return new Promise(async function(resolve, reject){
            try {
                MongoClient.connect(settings.mongoConnectionString, { poolSize : settings.poolSize, useUnifiedTopology: true }, async function(err, client) {
                    ...
                    await db.collection(constants.TABLENAME_CONTACTLOGS).createIndex( { 'receiverContext' : 1, 'type' : 1, 'eventContext' : 1 }, { unique: false, name : `${constants.TABLENAME_CONTACTLOGS}_unique` })
