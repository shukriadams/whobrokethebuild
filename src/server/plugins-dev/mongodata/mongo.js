        ...
        return new Promise(async function(resolve, reject){
            try {
                MongoClient.connect(settings.mongoConnectionString, { poolSize : settings.poolSize, useUnifiedTopology: true }, async function(err, client) {
                    ...
                    let db = client.db(settings.mongoDBName)
                    ...
                    await db.collection(constants.TABLENAME_BUILDINVOLVEMENTS).createIndex( { 'buildId' : 1, 'externalUsername' : 1 }, { unique: true, name : `${constants.TABLENAME_BUILDINVOLVEMENTS}_unique` })
